...
| Area | Strategy |
|---|---|
| Adaptive chunks | Dynamically adjust chunk size (1–8 MB) based on measured network throughput |
| Delta sync | Live Folders: rsync-like diff — transfer only changed bytes, not whole files |
| Streaming hash | Compute BLAKE3 hash while streaming (single pass), not as a separate post-transfer step |
| Transfer queue | Batch queue with priority ordering (small files first for UX responsiveness) |
| Backpressure | TCP window + HTTP flow control to prevent overwhelming slower receivers |
...
| Decision | Rationale |
|---|---|
| **XChaCha20 over AES-256-GCM** | `sodium_libs` Dart API has no high-level AES-GCM class. XChaCha20 has 192-bit nonces (safe for random), constant-time, NEON/SIMD accelerated. |
| **SodiumSumo over Sodium** | Standard `Sodium` lacks `scalarmult` (needed for X25519 ECDH). SodiumSumo includes full API. |
| **Plain Dart models over Freezed** | Eliminates `build_runner` + `freezed` + `json_serializable` + `freezed_annotation` + `json_annotation`. Avoids `analyzer ^9.0.0` conflict with Flutter SDK's pinned `test_api`. |
| **Manual Riverpod over @riverpod** | Eliminates `riverpod_generator` + `riverpod_annotation` + `build_runner`. Same Notifier/NotifierProvider pattern, zero code-gen dependencies. |
| **sqflite over Isar** | Isar's analyzer version pins conflicted with freezed/riverpod. sqflite + sqflite_common_ffi works cleanly on Android (native plugin) and Linux/Windows (FFI). |
| **Shelf over raw HttpServer** | Cleaner middleware pipeline, simpler routing, easier to test. Runs in Dart Isolate. |
| **Bonsoir 6.x sealed event API** | Events are sealed classes (`BonsoirDiscoveryServiceFoundEvent`, etc.), not enums. Pattern-matched in Dart 3. |
| **Feature-first folder layout** | Each feature owns its providers, screens, and widgets. Services are shared. Scales to large codebases. |
| **flutter_animate over raw AnimationController** | Declarative `.animate()` chains reduce boilerplate ~80%. GPU-composited via `RepaintBoundary`. Custom `AnimationController` still used for radar rings (continuous). |
| **Inter font over system default** | Closest OSS match to SF Pro. Consistent cross-platform rendering. Loaded lazily via `google_fonts` (cached on disk). |
| **No glassmorphism BackdropFilter on scrollables** | `BackdropFilter` triggers `saveLayer` per frame — costly on low-end Android. Reserved for static overlays only (modals, dialogs). |
| **Cupertino page transitions on all platforms** | iOS-like slide transitions feel polished and consistent. Configured in `PageTransitionsTheme`. |
...
| Task | Status |
|---|---|
| Architecture design & Blink.md | ✅ Complete |
| Dependency resolution & pubspec.yaml | ✅ Complete (zero code-gen deps) |
| Scaffold all 54 Dart source files | ✅ Complete |
| Models: plain Dart classes with copyWith/toJson/fromJson/equality | ✅ Complete (5 models) |
| Providers: manual Riverpod Notifier + NotifierProvider | ✅ Complete (7 providers) |
| Repositories: sqflite CRUD with row mappers | ✅ Complete (3 repositories) |
| SQLite schema: 5 tables via IsarService | ✅ Complete |
| Native FFI stubs: BLAKE3 + LZ4 headers + CMakeLists | ✅ Complete |
| Service stubs: all 12 services scaffolded | ✅ Complete |
| Screen stubs: all 9 screens + 4 widgets scaffolded | ✅ Complete |
| GoRouter: 9 routes configured | ✅ Complete |
| UI packages: animation, typography, icons, loading | ✅ Complete (8 packages) |
| Design system: colors, typography, spacing, shadows, animations, theme | ✅ Complete (6 token files) |
| `flutter analyze` — 0 errors, 0 warnings, 0 infos | ✅ Clean |

### Phase 1 — Core Transfer Engine

| Task | Status |
|---|---|
| Shelf HTTP server in Isolate with chunked streaming | Not started |
| HTTP client with PUT streaming + Content-Range headers | Not started |
| XChaCha20-Poly1305-IETF encrypt/decrypt integration | Not started |
| Pause/resume via HTTP Range headers | Not started |
| Transfer isolate pool with concurrency cap | Not started |
| Progress callback stream (throttled) | Not started |
| End-to-end send/receive on Linux desktop | Not started |

### Phase 2 — Discovery & Pairing

| Task | Status |
|---|---|
| mDNS advertise + discover with Bonsoir 6.x sealed events | Not started |
| BLE advertising + scanning on Android | Not started |
| DiscoveryManager: unified Device stream (mDNS + BLE) | Not started |
| QR handshake: generate → display → scan → validate → derive key | Not started |
| Ed25519 identity key persist to secure storage | Not started |
| X25519 ECDH session key derivation (SodiumSumo scalarmult) | Not started |

### Phase 3 — UI/UX Polish

| Task | Status |
|---|---|
| Design system tokens (colors, type, spacing, shadows, motion) | ✅ Complete |
| BlinkTheme.light() / .dark() full ThemeData builder | ✅ Complete |
| Onboarding flow: name + avatar + persist | Not started |
| Radar discovery screen with AnimationController + flutter_animate | Not started |
| Device bubble widgets with platform SVG icons | Not started |
| Transfer progress UI with TransferCard + animated ProgressBar | Not started |
| Skeleton loading states (shimmer) for discovery + transfer screens | Not started |
| Staggered list entrance animations | Not started |
| Lottie animations: loading, success, error, empty states | Not started |
| Adaptive desktop layout (wider screens) | Not started |
| Dark/light theme switching via SettingsProvider | Not started |

### Phase 4 — Advanced Features

| Task | Status |
|---|---|
| Chat: real-time text over transfer TCP channel | Not started |
| Live Folders: directory watcher → delta sync | Not started |
| Classroom Mode: one-to-many broadcast | Not started |
| Contact groups: persistent multi-device sharing | Not started |

### Phase 5 — Platform Hardening

| Task | Status |
|---|---|
| Android foreground service for background transfers | Not started |
| BLAKE3 native checksum integration (streamed) | Not started |
| LZ4 compression integration (adaptive skip for media) | Not started |
| Battery profiling on Android | Not started |
| Security audit: nonce uniqueness, key zeroing, timing attacks | Not started |
| WebRTC fallback end-to-end testing | Not started |
| Wi-Fi Hotspot fallback on Android | Not started |

---

## Key Implementation Notes

### Provider Pattern (Riverpod 3, no code-gen)

```dart
// Every provider follows this pattern:
class FooNotifier extends Notifier<FooState> {
  @override
  FooState build() => const FooState(); // initial state
...
class Foo {
  ...
  const Foo({this.id, required this.fooId});

  Foo copyWith({int? id, String? fooId}) => Foo(id: id ?? this.id, ...);

  Map<String, dynamic> toJson() => { ... };
  factory Foo.fromJson(Map<String, dynamic> json) => Foo(...);

  @override
  bool operator ==(Object other) => other is Foo && fooId == other.fooId;

  @override
  int get hashCode => fooId.hashCode;
}
```

### Bonsoir 6.x Event Handling

```dart
// Events are sealed classes, not enums. Use pattern matching:
...
```
encryptChunk(plaintext, sessionKey) →  [24B nonce || ciphertext || 16B Poly1305 tag]
decryptChunk(wire, sessionKey)      →  split at byte 24, decrypt remainder
```
